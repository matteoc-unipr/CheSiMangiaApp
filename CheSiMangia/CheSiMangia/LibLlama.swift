...
actor LlamaContext {
    ...
    func completion_init(text: String) {
        ...
        let needed = Int32(max(tokens_list.count + 8, 64))
        if needed > batchCapacity {
            llama_batch_free(batch)
            batch = llama_batch_init(needed, 0, 1)        // n_tokens=needed, embd=0, n_seq_max=1
            batchCapacity = needed
            print("[LLAMA] Reinit batch capacity to \(needed)")
        }
